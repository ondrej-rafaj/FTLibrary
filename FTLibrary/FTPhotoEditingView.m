...
#define kFTPhotoEditingViewInitialSpace                     70
#define kFTPhotoEditingViewAddToCenter                      8
...
- (CGPoint)retTopLeftPoint {
    ...
    p.x += kFTPhotoEditingViewAddToCenter;
    p.y += kFTPhotoEditingViewAddToCenter;
    return p;
}
...
- (void)createAllElements {
    ddTopLeft = [[FTPhotoEditingElementView alloc] initWithImage:[UIImage imageNamed:@"DD_photo_adjust_01.png"]];
    [ddTopLeft setCenter:CGPointMake(kFTPhotoEditingViewInitialSpace, kFTPhotoEditingViewInitialSpace)];
    [ddTopLeft addGestureRecognizer:[self getNewRecognizer]];
	[ddTopLeft setHidden:!_isCropEnabled];
    [self addSubview:ddTopLeft];
    
    ddTopRight = [[FTPhotoEditingElementView alloc] initWithImage:[UIImage imageNamed:@"DD_photo_adjust_02.png"]];
    [ddTopRight setCenter:CGPointMake((self.frame.size.width - kFTPhotoEditingViewInitialSpace), kFTPhotoEditingViewInitialSpace)];
    [ddTopRight addGestureRecognizer:[self getNewRecognizer]];
	[ddTopRight setHidden:!_isCropEnabled];
    [self addSubview:ddTopRight];
    
    ddBottomLeft = [[FTPhotoEditingElementView alloc] initWithImage:[UIImage imageNamed:@"DD_photo_adjust_02.png"]];
    [ddBottomLeft setCenter:CGPointMake(kFTPhotoEditingViewInitialSpace, (self.frame.size.height - kFTPhotoEditingViewInitialSpace))];
    [ddBottomLeft addGestureRecognizer:[self getNewRecognizer]];
	[ddBottomLeft setHidden:!_isCropEnabled];
    [self addSubview:ddBottomLeft];
    
    ddBottomRight = [[FTPhotoEditingElementView alloc] initWithImage:[UIImage imageNamed:@"DD_photo_adjust_01.png"]];
    [ddBottomRight setCenter:CGPointMake((self.frame.size.width - kFTPhotoEditingViewInitialSpace), (self.frame.size.height - kFTPhotoEditingViewInitialSpace))];
...
- (void)recalculateAllPositionsFrom:(FTPhotoEditingElementView *)v {
...
- (void)didMovePoint:(UIPanGestureRecognizer *)recognizer {
    FTPhotoEditingElementView *v = (FTPhotoEditingElementView *)recognizer.view;
...
- (void)drawRect:(CGRect)rect {
	...
	if (_isCropEnabled) {
		...
		CGContextMoveToPoint(context, ddTopLeft.center.x, ddTopLeft.center.y);
		CGContextAddLineToPoint(context, ddBottomLeft.center.x, ddBottomLeft.center.y);
		
		CGContextMoveToPoint(context, ddTopLeft.center.x, ddTopLeft.center.y);
		CGContextAddLineToPoint(context, ddTopRight.center.x, ddTopRight.center.y);
		
		CGContextMoveToPoint(context, ddTopRight.center.x, ddTopRight.center.y);
		CGContextAddLineToPoint(context, ddBottomRight.center.x, ddBottomRight.center.y);
		
		CGContextMoveToPoint(context, ddBottomRight.center.x, ddBottomRight.center.y);
		CGContextAddLineToPoint(context, ddBottomLeft.center.x, ddBottomLeft.center.y);
    }
