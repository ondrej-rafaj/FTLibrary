...
- (void)setMode:(FTProgressViewMode)newMode {
...
- (void)setProgress:(float)newProgress {
    ...
    if (mode == FTProgressViewModeDeterminate) {
		if ([NSThread isMainThread]) {
			[self updateProgress];
			[self setNeedsDisplay];
		} else {
...
- (void)updateProgress {
    [(FTRoundProgressView *)indicator setProgress:progress];
}
...
- (void)updateIndicators {
    ...
    if (mode == FTProgressViewModeDeterminate) {
        self.indicator = [[[FTRoundProgressView alloc] initWithDefaultSize] autorelease];
    }
    else if (mode == FTProgressViewModeCustomView && self.customView != nil){
...
+ (FTProgressView *)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
	FTProgressView *hud = [[FTProgressView alloc] initWithView:view];
	[view addSubview:hud];
	[hud show:animated];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
	...
	for (UIView *v in [view subviews]) {
		if ([v isKindOfClass:[FTProgressView class]]) {
			viewToRemove = v;
		}
	}
	if (viewToRemove != nil) {
		FTProgressView *HUD = (FTProgressView *)viewToRemove;
...
- (id)initWithView:(UIView *)view {
	...
	if (!view) {
		[NSException raise:@"FTProgressViewViewIsNillException" 
					format:@"The view used in the FTProgressView initializer is nil."];
	}
...
- (id)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        self.animationType = FTProgressViewAnimationFade;
        self.mode = FTProgressViewModeIndeterminate;
        self.labelText = nil;
...
- (void)layoutSubviews {
    ...
    if (nil != self.labelText) {
		...
		label.shadowColor = [UIColor colorWithRed:0.0 green:0.0 blue:0.0 alpha:0.15];
...
- (void)showUsingAnimation:(BOOL)animated {
    self.alpha = 0.0;
    if (animated && animationType == FTProgressViewAnimationZoom) {
    ...
    if (animated) {
        ...
        self.alpha = 1.0;
        if (animationType == FTProgressViewAnimationZoom) {
...
@implementation FTRoundProgressView
