...
#ifdef __IPHONE_OS_VERSION_MAX_ALLOWED
@interface FTCarouselView () <UIGestureRecognizerDelegate>
#else
@interface FTCarouselView ()
#endif
...
- (void)setDataSource:(id<FTCarouselViewDataSource>)_dataSource
{
...
- (void)setDelegate:(id<FTCarouselViewDelegate>)_delegate
{
...
- (void)setType:(FTCarouselViewType)_type
{
    if (type != _type)
    {
        type = _type;
...
{    
    ...
    switch (type)
    {
        case FTCarouselViewTypeLinear:
        {
            return CATransform3DTranslate(transform, offset * itemWidth, 0, 0);
        }
        case FTCarouselViewTypeRotary:
        case FTCarouselViewTypeInvertedRotary:
        {
            ...
            if (type == FTCarouselViewTypeInvertedRotary)
            {
        ...
        case FTCarouselViewTypeCylinder:
        case FTCarouselViewTypeInvertedCylinder:
        {
			NSInteger count = numberOfItems + (shouldWrap? 0: numberOfPlaceholders);
            ...
            if (type == FTCarouselViewTypeInvertedCylinder)
            {
                view.layer.doubleSided = NO;
                radius = -radius;
                angle = -angle;
        ...
        case FTCarouselViewTypeCoverFlow:
        {
            float tilt = 0.9;
            float spacing = 0.25;
...
{
    FTCarouselView *carousel = context;
...
- (void)scrollWheel:(NSEvent *)theEvent
{
	...
	//the FTCarouselView deceleration system conflicts with the built-in momentum
