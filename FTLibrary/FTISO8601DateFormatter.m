...
#import "FTISO8601DateFormatter.h"
...
@interface FTISO8601DateFormatter(UnparsingPrivate)
...
@implementation FTISO8601DateFormatter
...
- (id) init {
	if ((self = [super init])) {
		...
		format = FTISO8601DateFormatCalendar;
...
- (NSDate *) dateFromString:(NSString *)string timeZone:(out NSTimeZone **)outTimeZone range:(out NSRange *)outRange {
	NSTimeZone *timeZone = nil;
	if (!string) string = @"1970-01-01T01:01:01+0000";
...
- (NSString *) stringFromDate:(NSDate *)date timeZone:(NSTimeZone *)timeZone {
	switch (self.format) {
		case FTISO8601DateFormatCalendar:
			return [self stringFromDate:date formatString:ISO_CALENDAR_DATE_FORMAT timeZone:timeZone];
		case FTISO8601DateFormatWeek:
			return [self weekDateStringForDate:date timeZone:timeZone];
		case FTISO8601DateFormatOrdinal:
			return [self stringFromDate:date formatString:ISO_ORDINAL_DATE_FORMAT timeZone:timeZone];
		default:
			[NSException raise:NSInternalInconsistencyException format:@"self.format was %d, not calendar (%d), week (%d), or ordinal (%d)", self.format, FTISO8601DateFormatCalendar, FTISO8601DateFormatWeek, FTISO8601DateFormatOrdinal];
			return nil;
	}
}
