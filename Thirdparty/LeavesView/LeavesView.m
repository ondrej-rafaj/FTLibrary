...
@synthesize delegate;
@synthesize mode;
@synthesize currentPageIndex, backgroundRendering;
@synthesize interactionLocked;
@synthesize leafEdge;
@synthesize lockViewShadow;

- (UIImage *)getImageForPageAtIndex:(NSInteger)index
{
	UIImage *image = [[[UIImage alloc] initWithCGImage:[pageCache cachedImageForPageIndex:index]] autorelease];
	return image;
}
...
- (void)getImages {
    if (self.mode == LeavesViewModeSinglePage) {
        ...
        if (currentPageIndex <= numberOfPages) {
			...
			//topPage.contents = topPage.contents;
            topPage.contents = (id)[pageCache cachedImageForPageIndex:currentPageIndex];
...
#pragma mark Shadow lock

- (void)removeLockViewShadow {
    [lockViewShadow removeFromSuperview];
    [lockViewShadow release];
    lockViewShadow = nil;
}

- (void)lockView:(BOOL)show {
    if (show && lockViewShadow == nil) {
        lockViewShadow = [[UIView alloc] initWithFrame:self.bounds];
        [lockViewShadow setBackgroundColor:[UIColor colorWithRed:0 green:0 blue:0 alpha:0.75]];
        [lockViewShadow setAlpha:0];
        [lockViewShadow setAutoresizingMask:UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight];
        [self addSubview:lockViewShadow];
        
        [UIView beginAnimations:nil context:nil];
        [lockViewShadow setAlpha:1];
        [UIView commitAnimations];
    }
    else if (lockViewShadow != nil) {
        [UIView beginAnimations:nil context:nil];
        [UIView setAnimationDelegate:self];
        [UIView setAnimationDidStopSelector:@selector(removeLockViewShadow)];
        [lockViewShadow setAlpha:0];
        [UIView commitAnimations];
    }
}
...
- (void)setLeafEdge:(CGFloat)aLeafEdge {
	...
	CGFloat shadowOpacity = pageOpacity;
	
	if (aLeafEdge < 0.1) {
		shadowOpacity = (((aLeafEdge * 100) / 0.1) / 100);
	}
	
	topPageShadow.opacity			= pageOpacity;
	topPageReverseShading.opacity	= shadowOpacity;
	bottomPageShadow.opacity		= shadowOpacity;
...
- (void)setMode:(LeavesViewMode)newMode {
    ...
    if (NO) mode = LeavesViewModeSinglePage;
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    BOOL tapped = (distance(touchPoint, touchBeganPoint) < 10);
    BOOL nextPage = ((dragged && (touchBeganPoint.x > (self.frame.size.width - [self dragThreshold]))) || (tapped &&[self touchedNextPage]));
    BOOL prevPage = ((dragged && (touchBeganPoint.x < [self dragThreshold])) || (tapped && [self touchedPrevPage]));
    
    [CATransaction begin];
	float duration = 0.3;
	if (nextPage || (!prevPage && self.leafEdge < 0.5)) {   
	...
	else if (prevPage || (!nextPage && self.leafEdge > 0.5)) {
...
- (void)layoutSubviews {
	...
	if (!CGSizeEqualToSize(pageSize, desiredPageSize)) {
		...
		nextPageRect = CGRectMake(self.bounds.size.width - touchRectsWidth, 0, touchRectsWidth, self.bounds.size.height);
		prevPageRect = CGRectMake(0, 0, touchRectsWidth, self.bounds.size.height);
	}
}
