...
const NSUInteger kMaxDiskCacheSize			= 8e7;

static FTDiskCache *sharedInstance;

@interface FTDiskCache (Privates)
...
- (NSString *)cacheDir {
	if (_cacheDir == nil) {
		_cacheDir = [FTFilesystemPaths getImagesDirectoryPath];
...
- (void)trimDiskCacheFilesToMaxSize:(NSUInteger)targetBytes {
	...
	if ([self sizeOfCache] > targetBytes) {
		NSLog(@"time to clean the cache! size is: %@, %d", [self cacheDir], [self sizeOfCache]);
